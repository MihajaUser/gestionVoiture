...
<h2 class="h4 pd-20"> Vehicule Disponible</h2>
<table class="table">
	<thead class="thead-dark">
		<tr>
			...
			<th>Modele</th>
			<th>Type</th>
      <th>Assurance validite </th>
      <th>Visite validite </th>
			<?php
			?>
			<th></th>
		</tr>
	...
	<tbody> 
		<?php foreach ($vehicules as $vehicule) {?>
				...
				<td><?= $vehicule['type'] ?></td>
        <td><?= $vehicule['assurance_reste_jour'] ?></td>
        <td><?= $vehicule['visiteTechniques_reste_jour'] ?></td>
				<?php if ($user[0]['statut'] == "admin") { ?>
					<td>
						<form action="<?php echo site_url('') ?>trajets-vehicule-voiture-gestion-de-voiture" method="post">
							...
							<input type="submit" class="btn btn-secondary" value="Tous les trajets">
						</form>
					</td>
				<?php		} ?>
				</tr>
